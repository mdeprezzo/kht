  ...
  <div class="space-y-6">
    ...
    <div>
      <template v-if="!hideLabel">
        ...
        <label v-else class="mb-1.5 block text-sm font-medium text-gray-700">
          Upload file
        </label>      
      </template>
      <input
        ...
        @input="$emit('selected', ($event.target as HTMLInputElement).files?.[0])"
      />
    </div>
  </div>
</template>
...
withDefaults(defineProps<FileInputProps>(), {
  hideLabel: true,
})
