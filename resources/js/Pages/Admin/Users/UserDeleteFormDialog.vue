<template>
    <Modal :show="show">
        <div class="relative w-full rounded-3xl bg-white p-6 lg:p-10">
          ...
          <div class="space-y-4">
            <p class="text-sm text-gray-800">Sei sicuro di voler cancellare l'utente <strong>{{ itemToDelete?.name }}</strong>?</p>
          </div>
...
const remove = () => {
  form.delete(route('admin.users.destroy', { user: itemToDelete.value.id }), {
    onFinish: () => close()
  })
}
